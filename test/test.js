...
import { extractPNG, parsePNG } from '../dist/index.js';
const calcCRC32 = (chunk) => CRC32.buf(Buffer.concat([Buffer.from(chunk.type), chunk.data]));
// test image is `ct1n0g04.png` which includes `tEXt` chunk.
// http://www.libpng.org/pub/png/pngsuite.html
// http://www.libpng.org/pub/png/PngSuite/ct1n0g04.png
test('test fast-png-parser', async (t) => {
    await t.test('test extractPNG', async (_) => {
        const fh = await open('ct1n0g04.png', 'r');
        const chunks = await extractPNG(fh);
        fh.close();
        assert.strictEqual(chunks.length, 10);
        if (!chunks[0] || !chunks[9])
            return;
    ...
    await t.test('test filter of extractPNG', async (_) => {
        const fh = await open('ct1n0g04.png', 'r');
    ...
    await t.test('test maxChunks of extractPNG', async (_) => {
        const fh = await open('ct1n0g04.png', 'r');
    ...
    await t.test('test parsePNG', async (_) => {
        const fh = await open('ct1n0g04.png', 'r');
        const chunks = await extractPNG(fh);
        fh.close();
        assert.strictEqual(chunks.length, 10);
